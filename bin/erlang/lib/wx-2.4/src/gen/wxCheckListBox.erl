...
-export([append/2,append/3,appendStrings/2,appendStrings/3,cacheBestSize/2,
  ...
  disconnect/3,dragAcceptFiles/2,enable/1,enable/2,findString/2,findString/3,
...
new(#wx_ref{type=ParentT}=Parent,Id, Options)
 when is_integer(Id),is_list(Options) ->
  ...
  wxe_util:queue_cmd(Parent,Id, Opts,?get_env(),?wxCheckListBox_new_3),
...
check(#wx_ref{type=ThisT}=This,Item, Options)
...
isChecked(#wx_ref{type=ThisT}=This,Item)
 when is_integer(Item) ->
  ?CLASS(ThisT,wxCheckListBox),
  wxe_util:queue_cmd(This,Item,?get_env(),?wxCheckListBox_IsChecked),
...
%% @hidden
hitTest(This,X,Y) -> wxListBox:hitTest(This,X,Y).
%% @hidden
