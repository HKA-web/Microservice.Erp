    ...
    *,
    ...
    try:
        ...
        raise carehare.ChannelClosedByServer(
            405, "channelsrabbitmq refusing to redeclare queue %s" % queue_name
        )
...
channelsrabbitmq does not support Normal Channels. See
https://github.com/CJWorkbench/channelsrabbitmq/pull/11#issuecomment-499185070
...
class RabbitmqChannelLayer(BaseChannelLayer):
    ...
    in the channelsrabbitmq buffers before it is acked (and potentially not
...
If you're writing a django-channels Worker, channelsrabbitmq does not support
...
`sync_to_async()`, but `channelsrabbitmq` *doesn't* -- hence this error."""
            )

        if group_expiry is not None:
            warnings.warn(
                (
                    "channelsrabbitmq does not support group_expiry. Please do not configure it. "
                    "For rationale, see "
                    "https://github.com/CJWorkbench/channelsrabbitmq/issues/18#issuecomment-547052373"
                ),
    ...
    def carehare_connection(self) -> asyncio.Future[carehare.Connection]:
        ...
        if self._carehare_connection.get_loop() != asyncio.get_running_loop():
            raise RuntimeError(
                "The caller tried using channelsrabbitmq on a different event loop than the one it was initialized with."
            )
