...
class DjangoHueySettingsReader:
    def __init__(self, hueys_setting):
        ...
        if (
            ...
            raise ConfigurationError(
                f"Queue defined as default: {self.default_queue}, is not configured in HUEYS."
            )
        self.hueys = {}
    ...
    def configure(self):
        ...
        for queue_name, config in self.hueys_setting.items():
            ...
            if name in queue_names:
                raise ConfigurationError(
                    f"There are more than one queue with the name '{name}'. Check HUEYS in your settings file."
                )
    ...
    def _raise_queue_config_error(self):
        raise ConfigurationError(
            """
Command djangohuey must receive a --queue parameter or define a default queue in HUEYS setting.
i.e.:
