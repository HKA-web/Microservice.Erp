...
from drfapilogger.utils import database_log_enabled

if database_log_enabled():
    from drfapilogger.models import APILogsModel
    ...
    class APILogsAdmin(admin.ModelAdmin, ExportCsvMixin):
        ...
        change_list_template = 'drfapilogger/charts_change_list.html'
        change_form_template = 'drfapilogger/change_form.html'
        ...
        def get_queryset(self, request):
            drfapilogger_default_database = 'default'
            if hasattr(settings, 'DRF_API_LOGGER_DEFAULT_DATABASE'):
                drfapilogger_default_database = settings.DRF_API_LOGGER_DEFAULT_DATABASE
            return super(APILogsAdmin, self).get_queryset(request).using(drfapilogger_default_database)
        ...
        def changeform_view(self, request, object_id=None, form_url='', extra_context=None):
            if request.GET.get('export', False):
                ...
                if hasattr(settings, 'DRF_API_LOGGER_DEFAULT_DATABASE'):
                    drfapilogger_default_database = settings.DRF_API_LOGGER_DEFAULT_DATABASE
                export_queryset = self.get_queryset(request).filter(pk=object_id).using(drfapilogger_default_database)
