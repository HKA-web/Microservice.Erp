...
from drfapilogger import API_LOGGER_SIGNAL
from drfapilogger.start_logger_when_server_starts import LOGGER_THREAD
from drfapilogger.utils import get_headers, get_client_ip, mask_sensitive_data
...
class APILoggerMiddleware:
    def __init__(self, get_response):
        ...
        self.DRF_API_LOGGER_PATH_TYPE = 'FULL'
        if hasattr(settings, 'DRF_API_LOGGER_PATH_TYPE'):
            if settings.DRF_API_LOGGER_PATH_TYPE in ['ABSOLUTE', 'RAW', 'FULL']:
    ...
    def __call__(self, request):
        ...
        if self.DRF_API_LOGGER_DATABASE or self.DRF_API_LOGGER_SIGNAL:
            ...
            if response.get('content-type') in ('application/json', 'application/vnd.api+json',):
                ...
                elif self.DRF_API_LOGGER_PATH_TYPE == 'FULL':
                    api = request.get_full_path()
                elif self.DRF_API_LOGGER_PATH_TYPE == 'RAW':
