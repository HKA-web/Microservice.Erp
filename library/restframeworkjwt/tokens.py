...
from .blacklist.models import BlacklistedToken, OutstandingToken
from .utils import aware_utcnow, datetime_from_epoch, datetime_to_epoch, format_lazy

if TYPE_CHECKING:
    from .backends import TokenBackend

AuthUser = TypeVar("AuthUser", Any, Any)
    ...
    def token_backend(self) -> "TokenBackend":
        if self._token_backend is None:
            self._token_backend = import_string(
                "restframeworkjwt.state.token_backend"
            )
...
class BlacklistMixin:
    """
    If the `library.restframeworkjwt.blacklist` app was configured to be
    ...
    if "library.restframeworkjwt.blacklist" in settings.INSTALLED_APPS:
