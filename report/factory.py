...
from report.exceptions import UnsavedSchemaError
...
class ModelFactory:
    def __init__(self, model_schema):
        from report.models import Schema
        self.schema: Schema = model_schema
        self.registry = ModelRegistry(model_schema.app_label)

    def get_model(self):
        if not self.schema.pk:
            raise UnsavedSchemaError(f'''Cannot create a model for schema '{self.schema.schema_name}' because it has not been saved to the database''')
    ...
    def _model_meta(self):
        class Meta:
            ...
            verbose_name = self.schema.schema_name
...
class FieldFactory:
    def __init__(self, schema_field):
        from report.models import SchemaField
        self.schema: SchemaField = schema_field
